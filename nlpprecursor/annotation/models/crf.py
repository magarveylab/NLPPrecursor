...
class ConditionalRandomField(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 transition_of_interest: Tuple[int,int] = None) -> None:
        super().__init__()
        self.num_tags = num_tags
        self.transition_of_interest = transition_of_interest
    ...
    def get_transition_ohe(self,
        ...
        lists = []
        toi = self.transition_of_interest

        score = 0.

        for i in range(batch_size):
            lists.append((torch.zeros((sequence_length-1), dtype=torch.long),
                          torch.zeros((sequence_length-1,2))))
            for j in range(sequence_length-1):
                ...
                next_tag = tags[i][j+1]
                if (current_tag, next_tag) == toi:
                    lists[i][0][j] = 1
                else:
                    lists[i][0][j] = 0
                logit_start_tag = logits[i][j][toi[0]]
                logit_next_tags = logits[i][j+1][toi[1]]

                logit_other_start_tag = torch.sum(logits[i][j]) - logit_start_tag
                logit_other_next_tag = torch.sum(logits[i][j+1]) - logit_next_tags
                difference = logit_next_tags - logit_start_tag
                difference_other = logit_other_next_tag - logit_other_start_tag
                lists[i][1][j][0] = difference_other
                lists[i][1][j][1] = difference

        return lists

    def predict_transition_ohe(self, logits, mask):
        ...
        for i in range(batch_size):
            ...
            for j in range(sequence_length-1):
                ...
                difference = logit_next_tags - logit_start_tag
                difference_other = logit_other_next_tag - logit_other_start_tag
                pred_difference = difference / difference_other
                lists[i][1][j] = pred_difference

    def get_transition_loss(self, lists):
        score = 0.
        for compar in lists:
            score += self.loss_fn(compar[1], compar[0])
        return score
    ...
    def forward(self,
        ...
        #specific_score = self.get_transition_ohe(inputs, tags, mask)

        log_denominator = self._input_likelihood(inputs, mask)
        log_numerator = self._joint_likelihood(inputs, tags, mask)

        return torch.sum(log_numerator - log_denominator)# + specific_score
